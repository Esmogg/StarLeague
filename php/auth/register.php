...
class Usuario {
    ...
    public function existeUsuario(string $user, string $mail): bool {
        $sql = "SELECT id FROM usuario WHERE nombre = :user OR email = :mail LIMIT 1";
    ...
    public function registrar(string $user, string $mail, string $hash): bool {
        $sql = "INSERT INTO usuario (username, email, password_hash) VALUES (:user, :mail, :pass)";
        $stmt = $this->bd->prepare($sql);
...
try {
    ...
    $gestionUsuarios = new Usuario($pdo);
