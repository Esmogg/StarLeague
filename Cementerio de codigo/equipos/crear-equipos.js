...
async function enviarFormulario(event) {
  ...
  try {
    ...
    /* Enviar equipo al backend (antes esto se guardaba directo en localStorage) */

    const respuesta = await fetch("../../php/crear_equipo.php", {
      method: "POST",
    ...
    console.log("Equipo creado:", resultado.data);

    /* Si algún integrante no se pudo vincular (jugadores de prueba que no
       existen todavía como usuarios reales en la BD), avisamos por consola
       sin arruinar la creación del equipo */

    if (resultado.data?.advertencias?.length) {
      console.warn("Advertencias al vincular integrantes:", resultado.data.advertencias);
    }
